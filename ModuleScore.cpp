...
int ModuleScore::Load(const char* texture_path, const char* characters, uint rows)
{
	int id = -1;
...
void ModuleScore::UnLoad(int font_id)
{
...
void ModuleScore::Blit(int x, int y, int font_id, const char* text) const
{
...
update_status ModuleScore::Update()
{
	...
	App->score->Blit(16, 48, font_score, score_text);
	sprintf_s(score_text, 10, "%06i", App->scene_intro->points);
	App->score->Blit(16, 96, font_score, score_text);
	sprintf_s(score_text, 10, "%02i", App->scene_intro->numBalls);
	App->score->Blit(48, 192, font_score, score_text);
